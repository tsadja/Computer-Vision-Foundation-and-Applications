...
def lucas_kanade(img1, img2, keypoints, window_size=5):
    ...
    for y, x in keypoints:
        ...
        ### YOUR CODE HERE
        window_y = Iy[y-w:y+w+1,x-w:x+w+1].flatten()
        window_x = Ix[y-w:y+w+1,x-w:x+w+1].flatten()
        window_t = It[y-w:y+w+1,x-w:x+w+1].flatten()
        
        AT = np.array([window_y, window_x])
        A = AT.T
        b = -window_t.reshape(-1,1)
               
        flow_vector = np.linalg.inv(AT @ A) @ (AT @ b)
        flow_vectors.append(flow_vector.flatten())
        ### END YOUR CODE
...
def iterative_lucas_kanade(img1, img2, keypoints, window_size=9, num_iters=7, g=None):
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        ### YOUR CODE HERE
        window_y = Iy[y1-w:y1+w+1,x1-w:x1+w+1].flatten()
        window_x = Ix[y1-w:y1+w+1,x1-w:x1+w+1].flatten()
        
        AT = np.array([window_x, window_y])
        A = AT.T
        
        Ginv = np.linalg.inv(AT @ A)
        ### END YOUR CODE
        ...
        for k in range(num_iters):
            ...
            ### YOUR CODE HERE
            dIk = (img1[y1-w:y1+w+1,x1-w:x1+w+1] - img2[y2-w:y2+w+1,x2-w:x2+w+1]).flatten()
            bk = np.array([np.dot(dIk, window_x), np.dot(dIk, window_y)])
            vk = Ginv @ bk
            ### END YOUR CODE
...
def pyramid_lucas_kanade(
    ...
    for L in range(level, -1, -1):
        ...
        img1 = pyramid1[L]
        img2 = pyramid2[L]
        scaled_keypts = keypoints/(scale**L)
        d = iterative_lucas_kanade(img1, img2, scaled_keypts, window_size, num_iters, g)
...
def IoU(bbox1, bbox2):
    ...
    ### YOUR CODE HERE
    inter_ht = min(x1+w1, x2+w2) - max(x1,x2) 
    inter_wt = min(y1+h1, y2+h2) - max(y1,y2)
    inter_area = inter_ht * inter_wt

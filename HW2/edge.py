...
def non_maximum_suppression(G, theta):
    ...
    #print(G)
    ### BEGIN YOUR CODE
    padded = np.pad(G, ((1, 1), (1, 1)), 'constant')
    for i in range(H):
        for j in range(W):
            x, y = i+1, j+1
            curr = padded[x, y]
            if theta[i,j]==90 or theta[i,j]==270:
                next1 = padded[x-1, y] 
                next2 = padded[x+1, y]
            elif theta[i,j]==0 or theta[i,j]==180:
                next1 = padded[x, y+1] 
                next2 = padded[x, y-1]
            elif theta[i,j] == 45 or theta[i,j] == 225:
                next1 = padded[x-1, y-1] 
                next2 = padded[x+1, y+1]
            elif theta[i,j] == 135 or theta[i,j] == 315:
                next1 = padded[x-1, y+1] 
                next2 = padded[x+1, y-1]
            if curr>=next1 and curr>=next2:
                out[i,j] = curr           
    ### END YOUR CODE
...
def link_edges(strong_edges, weak_edges):
    ...
    ### YOUR CODE HERE
    strong_indices = list(map(tuple, indices))
    while len(strong_indices):
        (y,x) = strong_indices[0]
        strong_indices.pop(0)
        neighbors = get_neighbors(y,x,H,W)
        for neighbor in neighbors:
            if weak_edges[neighbor] and not edges[neighbor]:
                edges[neighbor] = 1
                strong_indices.append(neighbor)
    ### END YOUR CODE

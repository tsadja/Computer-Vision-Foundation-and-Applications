...
def resize_image(input_image, output_rows, output_cols):
    ...
    for i in range(output_rows):
        for j in range(output_cols):
            input_i = int(row_scale_factor*i)
            input_j = int(col_scale_factor*j)
...
def rotate2d(point, theta):
    ...
    return np.dot(rot, point).squeeze()
    ### END YOUR CODE
...
def rotate_image(input_image, theta):
    ...
    for i in range(input_rows):
        for j in range(input_cols):
                ...
                (input_i, input_j) = rotate2d(curr_pos-center, theta).astype(int) + center
                if input_i not in range(input_rows) or input_j not in range(input_cols):
                    output_image[i, j, :] = np.zeros((3,))
                else:
                    output_image[i, j, :] = input_image[input_i, input_j, :]
    ### END YOUR CODE

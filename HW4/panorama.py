...
def harris_corners(img, window_size=3, k=0.04):
    ...
    ### YOUR CODE HERE  
    Mxx = convolve(dx**2, window, mode='constant', cval=0)
    Myy = convolve(dy**2, window, mode='constant', cval=0)
    Mxy = convolve(dx*dy, window, mode='constant', cval=0)
    
    detM = (Mxx*Myy) - (Mxy**2)
    trM = Mxx + Myy
    
    response = detM - k*(trM**2)
    ### END YOUR CODE
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    ### YOUR CODE HERE 
    dists_sort = np.sort(dists, axis=1)[:,0:2]
    idx = np.argsort(dists, axis=1)[:,0]

    for i in range(M):
        ratio = dists_sort[i,0]/dists_sort[i,1]
        if ratio < threshold:
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for i in range(n_iters):
        ...
        eucdist = np.linalg.norm((matched2 @ H) - matched1, axis=1)**2
        inliers_idx = eucdist < threshold
        
        # 4. Repeat and keep the largest set of inliers (use >, i.e. break ties by whichever set is seen first)
        if np.sum(inliers_idx) > n_inliers:
            max_inliers = inliers_idx
            n_inliers = np.sum(inliers_idx)
    
    # 5. Re-compute least-squares estimate on all of the inliers
    sample1 = matched1[max_inliers]
    sample2 = matched2[max_inliers]
    H = np.linalg.lstsq(sample2, sample1, rcond=None)[0]
    H[:,2] = np.array([0, 0, 1])
        ### END YOUR CODE
...
def hog_descriptor(patch, pixels_per_cell=(8,8)):
    ...
    ### YOUR CODE HERE
    # 2. Compute gradient histograms for each cell
    p1 = pixels_per_cell[0]
    p2 = pixels_per_cell[1]
    for i in range(rows):
        for j in range(cols):
            for k in range(p1):
                for l in range(p2):
                    curr_bin = int(theta_cells[i,j,k,l] // degrees_per_bin) % n_bins
                    hists[i,j,curr_bin] += G_cells[i,j,k,l]
...
def linear_blend(img1_warped, img2_warped):
    ...
    ### YOUR CODE HERE
    # Correct left_margin and right margin to be more precise
    left_margin = np.min(np.where(img2_mask==1)[1])
    right_margin = np.max(np.where(img1_mask==1)[1])
    
    wt1 = np.ones_like(img1_warped)
    wt2 = np.ones_like(img2_warped)
    
    if left_margin < right_margin:
        N = right_margin - left_margin + 1
        wt1[:, left_margin:right_margin+1] = np.tile(np.linspace(1,0,N), (out_H,1))
        wt2[:, left_margin:right_margin+1] = np.tile(np.linspace(0,1,N), (out_H,1))
    
    merged = (wt1*img1_warped) + (wt2*img2_warped)
    ### END YOUR CODE
...
def stitch_multiple_images(imgs, desc_func=simple_descriptor, patch_size=5):
    ...
    # 1. Perform RANSAC
    Hs = []
    # First image has H = I_3, based on how we warped images in notebook
    Hs.append(np.eye(3))
    for i in range(len(keypoints)-1):
        kypnts1 = keypoints[i]
        kypnts2 = keypoints[i+1]
        # Use matches from first of 2 keypoints, based on notebook
        mtchs = matches[i]
        # Use same threshold = 1 as notebook
        H = ransac(kypnts1, kypnts2, mtchs, threshold=1)[0]
        Hs.append(H)
        
    for i in range(1, len(keypoints)):
        Hs[i] = Hs[i] @ Hs[i-1]
    
    # 2. Get output space, map images to ref image with transforms
    ...
    for i in range(len(keypoints)):
        ...
        # Reuse post-processing for warped image from notebook
    ...
    # 4. Merge the warped images using linear blending scheme
    panorama = imgs_warped[0]
    for i in range(1, len(imgs_warped)):
        img2_warped = imgs_warped[i]

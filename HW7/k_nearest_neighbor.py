...
def predict_labels(dists, y_train, k=1):
    ...
    for i in range(num_test):
        ...
        k_smallest = y_train[sort_dists][0:k]
        y_pred[i] = np.argmax(np.bincount(k_smallest))
    # END YOUR CODE
...
def split_folds(X_train, y_train, num_folds):
    ...
    X = np.array_split(X_train, num_folds)
    Y = np.array_split(y_train.reshape(-1,1), num_folds)
    
    for i in range(num_folds):
        X_trains[i,:,:] = np.vstack(X[:i] + X[i+1:])
        y_trains[i,:] = np.vstack(Y[:i] + Y[i+1:]).reshape(-1)
        X_vals[i,:,:] = X[i]
        y_vals[i,:] = Y[i].reshape(-1)
    # END YOUR CODE

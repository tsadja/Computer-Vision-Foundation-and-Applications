...
def sliding_window(image, template_feature, step_size, window_size, pixel_per_cell=8,
    ...
    for r in range(0, H, step_size):
        for c in range(0, W, step_size):
            score = 0
            ### YOUR CODE HERE
            img_window = pad_image[r:r+winH, c:c+winW]
            img_hog_feature, _ = hog_feature(img_window, pixel_per_cell)
            score = np.dot(img_hog_feature, template_feature)
...
def pyramid_score(image, template_feature, shape, step_size=20,
    ...
    for img_tuple in images:
        curr_scale, img = img_tuple
        (curr_max_score, curr_maxr, curr_maxc, curr_response_map_resized) = sliding_window(img, template_feature, step_size=step_size, window_size=shape, pixel_per_cell=pixel_per_cell)
        if curr_max_score > max_score:
            max_score = curr_max_score
            maxr = curr_maxr
            maxc = curr_maxc
            max_scale = curr_scale
            max_response_map = curr_response_map_resized
    ### END YOUR CODE
...
def gaussian_heatmap(heatmap_face, heatmaps, sigmas):
    ...
    ### YOUR CODE HERE
    heatmap = np.zeros(heatmaps[0].shape)
    for curr_heatmap, sigma in zip(heatmaps, sigmas):
        heatmap += gaussian(curr_heatmap, sigma)
